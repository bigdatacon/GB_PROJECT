...
#     b = tuple(b)
#     return geodesic(a, b).meters
""" для обработки действ точек включить тюплы!!!!!!!!!!!!"""
def distance_point_metro(a, b):
    # a = list(a)
    # b = list(b)
    return geodesic(a, b).meters
...
def lambda_high(multiple, massive):
    massive_list = []
    for i in massive:
...
def get_itog_dots_list_with_options(list):
    ...
    for i in list:
        ...
        if i.get("chisl_apprise") == 'good' and i.get('dist_to_conquer_aprise') == 'good' and i.get('dist_to_close_ost_apr')=='good':
            itog_dots.append(i)
...
def get_distances_less_then_need(res, dist, data_house):
    ...
    for i in res:
        if i<dist:
            ind = res.index(i)
            try:
                people = data_house[ind].get('Зарегистрировано жителей')
                dist_vaq = {'dist': i, 'colich_pupl': people}
                dist_list.append(dist_vaq)
                dist_sum.append(people)
...
""" функция по нахождению количества конкурентов в заданном радиусе и количеству людей которые на них приходятся"""
...
def get_info_newp_house(data_house, data_new_point, dist, dist_two_km, data_point):
    for i in tqdm(data_new_point):
           i['status'] = 'new'
           i['category'] = categ
           ...
           res_conq = lambda_high_new(multiple, data_point)
           sort_res_conq = sorted(res_conq)[0]
           res_ind_conq = res_conq.index(sort_res_conq)
           res_info = sort_res_conq
           i['количество жителей в радиусе до конкурента методом домов'] = get_distances_less_then_need(res, res_info, data_house)[1]
           i['chisl'] = (i['closer_metro_area_densyty']*3.14*res_info*res_info)/1000000
           i['количество конкурентов в радиусе 2 км'] = get_conquer_qantity_tw(res_conq, dist_two_km)
           i['dist_to_closest_conquer'] = res_info

           i['население на 1 конкурента в радиусе 2 км методом плотности'] = float((i['closer_metro_area_densyty']*3.14*2000*2000)/1000000) / float(i['количество конкурентов в радиусе 2 км']) if \
               float(i['количество конкурентов в радиусе 2 км']) >=1 else float((i['closer_metro_area_densyty']*3.14*2000*2000)/1000000)
           i['население на 1 конкурента в радиусе 2 км методом по домам'] = float(i['количество жителей в радиусе 2 км методом домов']) / float(i['количество конкурентов в радиусе 2 км']) if \
               float(i['количество конкурентов в радиусе 2 км']) >=1 else float(i['количество жителей в радиусе 2 км методом домов'])
           """оценочные параметры - плохая или хорошая оценка у точки"""
           dist_to_closest_conquer_mean, dist_to_closer_metro_mean, chisl_mean = get_midle_list_info(data_point, categ)
           i['dist_to_conquer_aprise'] = 'good' if res_info > dist_to_closest_conquer_mean*1.2 else 'bad'
           i['dist_to_closer_metro_aprise'] = 'good' if i['dist_to_closer_metro']/1.2 < dist_to_closer_metro_mean else 'bad'
           i['chisl_apprise'] = 'good' if i['chisl']  > chisl_mean*1.2 else 'bad'
           i['dist_to_close_ost_apr'] = 'good' if i['dist_to_closer_ostanovk'] <= float(600) else 'bad'
           i['среднее расстояние до метро по действ точкам'] = dist_to_closer_metro_mean
           i['среднее расстояние до конкурента по действ точкам'] = dist_to_closest_conquer_mean
           i['среднея численность в радиусе до конкурента по действ точкам'] = chisl_mean
           i['latitude'] = i['coordinates'][0]
           i['longitude'] = i['coordinates'][1]
           # del i['расстояние до ближайшего конкурента'], i['расстояние до конкурента']
